 ...
 * Version 1.3 - Fixes to channel parser. Auto conversion of Noteon(0) to Off
 *
...
#define _CVAR_CAST				const _MIDI_FILE *pMF = (const _MIDI_FILE *)_pMF
...
long size;
	...
	if (fp)
		...
		if ((pMF = (_MIDI_FILE *)malloc(sizeof(_MIDI_FILE))))
			...
			size = ftell(fp);
			if ((pMF->ptr = (BYTE *)malloc((unsigned long)size)))
				{
				fseek(fp, 0L, SEEK_SET);
				fread(pMF->ptr, sizeof(BYTE), (unsigned long)size, fp);
...
{
const MIDI_END_POINT *p1 = (const MIDI_END_POINT *)e1;
const MIDI_END_POINT *p2 = (const MIDI_END_POINT *)e2;
...
BOOL bNoChanges = TRUE;
	...
	*/
	...
	if (mx_pts)
		...
		qsort(pEndPoints, (unsigned int)mx_pts, sizeof(MIDI_END_POINT), qs_cmp_pEndPoints);
		...
		while ((dwEndTimePos >= (DWORD)pEndPoints[i].iEndPos || bFlushToEnd) && i<mx_pts)
			...
			ptr = _midiWriteVarLen(ptr, (int)(pMF->Track[iTrack].LastNote[num].end_pos - pMF->Track[iTrack].pos));
...
int dtime;
	...
	memcpy(ptr, pData, (unsigned int)data_sz);
...
{
	return midiTrackAddMsg(pMF, iTrack, msgControlChange, (int)iCCType, iParam);
}
...
int chn;
	...
	for(i=0;i<sizeof(pTrk->LastNote)/sizeof(pTrk->LastNote[0]);++i)
		if (pTrk->LastNote[i].valid == FALSE)
			...
			ptr = _midiWriteVarLen(ptr, (int)(pTrk->dt));		/* delta-time */
...
BOOL midiReadGetNextMessage(MIDI_FILE *_pMF, int iTrack, MIDI_MSG *pMsg)
{
...
int sz;
	...
	switch(pMsg->iType)
		...
		case	msgNoteOn:
							...
							if (pMsg->iType == msgNoteOn && *(pMsgDataPtr+1) == 0) {
								pMsg->iType = msgNoteOff;
		...
		case	msgMetaEvent:
							...
							** always have bit 7 set */
							...
							if (_midiReadTrackCopyData(pMsg, pTrack->ptr, (DWORD)sz, FALSE) == FALSE)
								return FALSE;

							/* Now copy the data...*/
							memcpy(pMsg->data, bptr, (unsigned int)sz);
	...
	*/
	if ((pMsg->iType&0xf0) != 0xf0)
		{
		/* non-sysex messages */
		if ((*pTrack->ptr & 0x80) == 0) {
			pMsg->iImpliedMsg = pMsg->iType;
			--pMsg->iMsgSize;
		}
